const PASS_KEY = "nursery_admin_pass";
...
els.loginBtn.addEventListener("click", () => {
  ...
  const savedPass = localStorage.getItem("nursery_admin_code");

  if (!savedPass && pass === DEFAULT_PASS) {
    localStorage.setItem("nursery_admin_code", DEFAULT_PASS);
...
els.form.addEventListener("submit", (e) => {
  ...
  const product = {
    ...
    image: els.prodImage.value.trim() || "https://images.unsplash.com/photo-1501004318641-b39e6451bec6?w=900&q=60",
...
els.exportOrdersBtn.addEventListener("click", () => {
  ...
  const rows = [
    ["order_id", "created_at", "name", "phone", "items", "total", "message"]
  ];
...
function renderProducts(products) {
  els.productsTable.innerHTML = products.map(product => `
    <tr class="border-b align-top">
      ...
      <td class="py-3 pr-3">
        ${product.featured ? '<span class="px-2 py-1 rounded-full text-xs bg-green-100 text-green-700">Yes</span>' : '<span class="px-2 py-1 rounded-full text-xs bg-slate-100 text-slate-600">No</span>'}
      </td>
      <td class="py-3 pr-3">
